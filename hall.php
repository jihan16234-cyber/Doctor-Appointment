...
<script>
    ...
    /**
 * Handles the final booking button click by preventing default form submission (redirect).
 * This keeps the user on ho.php and simulates a successful booking.
 */
function finalizeBookingForm(event) {
    // 1. CRITICAL STEP: Stops the page from redirecting or refreshing.
    event.preventDefault(); 
    
    // 2. (In a real system) You would use fetch() or AJAX here 
    //    to send the booking data to a server endpoint in the background.
    
    // 3. Close the modal and confirm success
    document.getElementById('ambulance-booking-modal').classList.add('hidden');
    
    // Optional: Reset progress bar back to step 1 (assuming this is step 3 or 4)
    showBookingStep(1); 
    
    alert('Ambulance Booking Finalized successfully! The page navigation to pay.php has been stopped.');
